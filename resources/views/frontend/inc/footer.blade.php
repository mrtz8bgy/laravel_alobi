{{-- Footer services bar --}}
@php $gs = \App\GeneralSetting::first(); @endphp

<footer class="d-footer">
    <div class="container">
        <div class="d-footer-grid">
            {{-- Column 1: Brand --}}
            <div class="d-footer-col d-footer-brand">
                <a href="{{ route('home') }}" class="d-logo" style="text-decoration:none;">
                    <div class="d-logo-mark">
                        @if($gs->logo != null && file_exists(public_path($gs->logo)))
                            <img src="{{ asset($gs->logo) }}" alt="Marketplace" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">
                        @else
                            M
                        @endif
                    </div>
                    <div class="d-logo-text">
                        <span class="brand-name">MARKETPLACE</span>
                        <span class="brand-tag">Multi-Vendor Store</span>
                    </div>
                </a>
                <p>
                    فروشگاه آنلاین ما یک بازار عمومی چندفروشنده‌ای است که محصولات متنوعی از دسته‌بندی‌های مختلف را در یک تجربه خرید یکپارچه ارائه می‌دهد.
                </p>
                <div class="d-footer-social">
                    @if($gs->instagram)<a href="{{ $gs->instagram }}" target="_blank" aria-label="Instagram"><i class="fa fa-instagram"></i></a>@endif
                    @if($gs->facebook)<a href="{{ $gs->facebook }}" target="_blank" aria-label="Facebook"><i class="fa fa-facebook"></i></a>@endif
                    @if($gs->twitter)<a href="{{ $gs->twitter }}" target="_blank" aria-label="Twitter"><i class="fa fa-twitter"></i></a>@endif
                    @if($gs->youtube)<a href="{{ $gs->youtube }}" target="_blank" aria-label="Youtube"><i class="fa fa-youtube-play"></i></a>@endif
                    @if($gs->google_plus)<a href="{{ $gs->google_plus }}" target="_blank" aria-label="Google+"><i class="fa fa-google-plus"></i></a>@endif
                    <a href="#" aria-label="Telegram"><i class="fa fa-telegram"></i></a>
                    <a href="#" aria-label="Whatsapp"><i class="fa fa-whatsapp"></i></a>
                </div>
            </div>

            {{-- Column 2: Links --}}
            <div class="d-footer-col">
                <h5>دسترسی سریع</h5>
                <ul class="d-footer-links">
                    <li><a href="{{ route('home') }}">صفحه اصلی</a></li>
                    <li><a href="{{ route('categories.all') }}">همه دسته‌بندی‌ها</a></li>
                    <li><a href="{{ route('products') }}">محصولات</a></li>
                    <li><a href="{{ route('brands.all') }}">برندها</a></li>
                    <li><a href="{{ route('products.featured') }}">محصولات ویژه</a></li>
                    <li><a href="{{ route('products.best_selling') }}">پرفروش‌ترین‌ها</a></li>
                    <li><a href="#">وبلاگ و مقالات</a></li>
                </ul>
            </div>

            {{-- Column 3: Services --}}
            <div class="d-footer-col">
                <h5>خدمات مشتریان</h5>
                <ul class="d-footer-links">
                    <li><a href="{{ route('orders.track') }}">رهگیری سفارش</a></li>
                    <li><a href="{{ route('sellerpolicy') }}">قوانین فروشندگان</a></li>
                    <li><a href="{{ route('returnpolicy') }}">شرایط بازگشت کالا</a></li>
                    <li><a href="{{ route('supportpolicy') }}">پشتیبانی</a></li>
                    <li><a href="{{ route('privacypolicy') }}">حریم خصوصی</a></li>
                    <li><a href="{{ route('terms') }}">قوانین استفاده</a></li>
                </ul>
            </div>

            {{-- Column 4: Contact + Newsletter --}}
            <div class="d-footer-col">
                <h5>ارتباط با ما</h5>
                <ul class="d-footer-contact">
                    <li><i class="la la-map-marker"></i> <span>{{ $gs->address ?? 'تهران، خیابان شریعتی، پلاک ۱۲۳' }}</span></li>
                    <li><i class="la la-phone"></i> <span>{{ $gs->phone ?? '۰۲۱-۱۲۳۴۵۶۷۸' }}</span></li>
                    <li><i class="la la-envelope"></i> <span>{{ $gs->email ?? 'info@marketplace.com' }}</span></li>
                    <li><i class="la la-clock-o"></i> <span>شنبه تا پنجشنبه، ۹ الی ۲۰</span></li>
                </ul>
                <div style="margin-top:18px;">
                    <div style="color: var(--d-gold); font-size:13px; font-weight:600; margin-bottom:8px;">
                        <i class="la la-bullhorn"></i> عضویت در خبرنامه
                    </div>
                    <form method="POST" action="{{ route('subscribers.store') }}" class="d-newsletter-form">
                        @csrf
                        <input type="email" name="email" placeholder="ایمیل شما..." required>
                        <button type="submit" aria-label="عضویت"><i class="la la-paper-plane la-flip-horizontal"></i></button>
                    </form>
                </div>
            </div>
        </div>

        {{-- Bottom bar --}}
        <div class="d-footer-bottom">
            <div>© {{ date('Y') }} {{ $gs->site_name ?? 'Marketplace' }} — تمامی حقوق محفوظ است.</div>
            <div class="pay-methods">
                <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='40' height='24' viewBox='0 0 40 24'><rect width='40' height='24' fill='%236b7280' rx='3'/><text x='50%25' y='56%25' font-family='sans-serif' font-size='9' fill='%23ffffff' text-anchor='middle' font-weight='900'>VISA</text></svg>" alt="Visa">
                <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='40' height='24' viewBox='0 0 40 24'><rect width='40' height='24' fill='%231f2937' rx='3'/><text x='50%25' y='56%25' font-family='sans-serif' font-size='8' fill='%23ffffff' text-anchor='middle' font-weight='900'>MC</text></svg>" alt="MasterCard">
                <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='40' height='24' viewBox='0 0 40 24'><rect width='40' height='24' fill='%233b82f6' rx='3'/><text x='50%25' y='56%25' font-family='sans-serif' font-size='8' fill='%23ffffff' text-anchor='middle' font-weight='900'>BANK</text></svg>" alt="Payment">
                @if(\App\BusinessSetting::where('type','cash_payment')->first()->value == 1)
                <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='40' height='24' viewBox='0 0 40 24'><rect width='40' height='24' fill='%23e5e7eb' rx='3'/><text x='50%25' y='56%25' font-family='sans-serif' font-size='7' fill='%230f172a' text-anchor='middle' font-weight='900'>Cash</text></svg>" alt="Cash on delivery">
                @endif
            </div>
        </div>
    </div>
</footer>
